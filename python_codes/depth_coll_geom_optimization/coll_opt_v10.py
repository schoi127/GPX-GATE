...
def objective(params):
    ...
    p5 = np.array([cg_L_x + w/2, cg_L_y + h_L / 2])
    p6 = np.array([cg_L_x + w/2, cg_L_y - h_L / 2])
    p7 = np.array([cg_R_x - w/2, cg_R_y + h_R / 2])
    p8 = np.array([cg_R_x - w/2, cg_R_y - h_R / 2])
    ...
    line1_dist = np.abs(np.cross(p3 - p1, p5_rot - p1)) / np.linalg.norm(p3 - p1) + \
                 np.abs(np.cross(p3 - p1, p8_rot - p1)) / np.linalg.norm(p3 - p1)
    line2_dist = np.abs(np.cross(p4 - p2, p7_rot - p2)) / np.linalg.norm(p4 - p2) + \
                 np.abs(np.cross(p4 - p2, p6_rot - p2)) / np.linalg.norm(p4 - p2)
...
def constraint2(params):
    ...
    # Calculate p5, p7
    p5 = rotate_point(np.array([cg_L_x + w/2, cg_L_y + h_L / 2]), [cg_L_x, cg_L_y], ang_L)
    p7 = rotate_point(np.array([cg_R_x - w/2, cg_R_y + h_R / 2]), [cg_R_x, cg_R_y], ang_R)
...
def constraint3(params):
    ...
    # Calculate p1, p6
    p1 = np.array([-r - (pitch / 2), det_y])
    p6 = rotate_point(np.array([cg_L_x + w/2, cg_L_y - h_L / 2]), [cg_L_x, cg_L_y], ang_L)

    # Calculate the slope of p1-p3 line
    slope = (p3[1] - p1[1]) / (p3[0] - p1[0])

    # Calculate y-coordinate on p1-p3 line at p6's x-coordinate
    y_on_line = slope * (p6[0] - p1[0]) + p1[1]

    # Constraint: p6 must be on or to the left of p1-p3 line
    return y_on_line - p6[1]
...
def constraint4(params):
    ...
    # Calculate p2, p8
    p2 = np.array([-r + (pitch / 2), det_y])
    p8 = rotate_point(np.array([cg_R_x - w/2, cg_R_y - h_R / 2]), [cg_R_x, cg_R_y], ang_R)

    # Calculate the slope of p2-p4 line
    slope = (p4[1] - p2[1]) / (p4[0] - p2[0])

    # Calculate y-coordinate on p2-p4 line at p8's x-coordinate
    y_on_line = slope * (p8[0] - p2[0]) + p2[1]

    # Constraint: p8 must be on p2-p4 line
    return abs(y_on_line - p8[1])
...
# Visualization code remains the same as in the original script
# ...
# Calculate p1, p2
...
# Calculate p5, p6, p7, p8
p5_opt = np.array([cg_L_x + w/2, cg_L_y + h_L / 2])
p6_opt = np.array([cg_L_x + w/2, cg_L_y - h_L / 2])
p7_opt = np.array([cg_R_x - w/2, cg_R_y + h_R / 2])
p8_opt = np.array([cg_R_x - w/2, cg_R_y - h_R / 2])
